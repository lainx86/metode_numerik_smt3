...
x = linspace(-2,4,100);
fx = x.^3 + x.^2 - 3*x - 3;
gx1 = (3*x + 3 - x.^2).^(1/3);   % Fungsi iterasi 1
gx2 = sqrt(3*x + 3 - x.^3);      % Fungsi iterasi 2 (alternatif)
...
xlabel('x')
ylabel('y')
title({'Grafik Metode Iterasi Fungsi x^3 + x^2 - 3x - 3'; ...
...
xn = 1; % Nilai awal
Error_max = 10^(-4); % Batas toleransi
...
while iterasi < maks_iterasi
    ...
    % Pilih fungsi iterasi
    g_xn = fungsi_iterasi_1(xn); % Gunakan g1(x)
    ...
    if iterasi > 1 && error <= Error_max
        ...
        legend('f(x)', 'y=0', 'g_1(x)', 'g_2(x)', ...
               'Akar Persamaan', 'Titik Iterasi', 'Location', 'best');
        return
    end
...
if iterasi >= maks_iterasi
    ...
    legend('f(x)', 'y=0', 'g_1(x)', 'g_2(x)', ...
           'Pendekatan Akhir', 'Titik Iterasi', 'Location', 'best');
end

% ====== Fungsi Pendukung ======
function fx = fungsi_utama(x)
    % Fungsi utama: f(x) = x^3 + x^2 - 3x - 3
    fx = x^3 + x^2 - 3*x - 3;
end

function gx = fungsi_iterasi_1(x)
    % Fungsi iterasi 1: g(x) = (3x + 3 - x^2)^(1/3)
    gx = (3*x + 3 - x^2)^(1/3);
end

function gx = fungsi_iterasi_2(x)
    % Fungsi iterasi 2 alternatif: g(x) = sqrt(3x + 3 - x^3)
    if (3*x + 3 - x^3) >= 0
        gx = sqrt(3*x + 3 - x^3);
    else
        gx = NaN;
