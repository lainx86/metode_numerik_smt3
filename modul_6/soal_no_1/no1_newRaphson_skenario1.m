...
xlabel('x')
ylabel('y')
title({'Grafik Metode Newton-Raphson Fungsi x^3-sin(x)-exp(x)';'Feby Syarief Al a`raaf-26050124130087-C'})
...
function [fx, fprime] = newton_raphson(x)
    % Fungsi: f(x) = x^3 - sin(x) - exp(x)
    fx = x^3 - sin(x) - exp(x);
    
    % Turunan: f'(x) = 3x^2 - cos(x) - exp(x)
    fprime = 3*x^2 - cos(x) - exp(x);
end
